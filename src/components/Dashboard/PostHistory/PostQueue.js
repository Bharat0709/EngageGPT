...
const PostQueue = () => {
  ...
  if (!selectedProfile && invitedProfiles.length > 0) {
    return (
      <div className="dashboard-container bg-[#fafafa] rounded-xl h-full ">
        <div className="flex lg:flex-row flex-wrap  p-2 px-4  items-center justify-center gap-3 mb-2 lg:justify-between">
        ...
        <div className="rounded-2xl m-2 text-center">
          <div className="flex flex-col items-center justify-center py-8">
            <img src={disconnected} alt="not-connected" className="h-60 w-60" />
            <h2 className="text-xl ovo-regular  font-semibold text-gray-800 mb-2">
              No LinkedIn Profiles Connected
            </h2>
            <button
              onClick={handleConnectLinkedIn}
              className="btn-primary mt-4 geist flex items-center gap-2 whitespace-nowrap px-6 py-2 text-sm font-medium bg-white border border-black text-black w-fit transition-all shadow-[3px_3px_0px_black] hover:shadow-none hover:translate-x-[3px] hover:translate-y-[3px]"
            >
