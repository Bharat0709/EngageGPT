...
const PostFilters = ({ posts, handleSortChange }) => {
  ...
  const filters = ['Date Posted','Likes', 'Shares', 'Views', 'Impressions', 'Comments'];

  function getOldestPostTime(posts) {
    // Helper function to extract months, years, days, weeks from the 'postedAround' field
    const extractTimeUnits = (time) => {
      const match = time.match(/(\d+)\s*(year|month|week|day|hour|minute)s?\s*ago/);
    ...
    posts.forEach(post => {
      ...
      if (amount < oldestAmount || (amount === oldestAmount && unit < oldestUnit)) {
        oldestAmount = amount;
        oldestUnit = unit;
      }
    });
  ...
  return (
    <div className="bg-white rounded-lg flex flex-col gap-2 py-3">
        <div className="toggle-buttons flex justify-between bg-white border border-gray-300 p-3 rounded-lg  gap-4">
          <p className='text-sm'>{posts.length} Posts from past {oldestPost}.</p>
